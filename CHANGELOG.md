...
- Le type "pays" gagne deux nouveaux modes de devinette (`questionType`) : `"map"` (zoom continu depuis la carte du monde jusqu'à la silhouette du pays cible, rendu 100 % vectoriel côté client à partir d'un fond de carte embarqué, sans clé ni serveur de tuiles) et `"leader"` (deviner le pays à partir du portrait et du nom de son chef d'État actuel, extrait de l'infobox de l'article Wikipédia du pays dans le dump local déjà utilisé par `wiki_article` — pas de repli API si le dump est indisponible).
- Nouveau type "chef d'État" (`type: "statesman"`) : le sens inverse de `country:leader` — deviner le chef d'État à partir du drapeau et du nom de son pays. Réutilise entièrement le pool et les filtres de `country` (pas de données propres) ; le chef d'État découvert devient une vraie entrée `person` taguée `role: "politician"`, donc il apparaît aussi dans le pool "personne"/politicien classique.
- Nouveau palier de popularité "Star" : ajouté à quasiment tous les types qui avaient déjà "Obscur"/"Niche" (films, séries, jeux, musique, personnes/acteurs/réalisateurs via leur liste "Populaire" existante ; peintres, rôles Wikidata, articles Wikipédia, super-héros et chefs d'État via un découpage en quartiles) — la moitié haute, en valeur, de ce qui était déjà "Populaire"/du haut du classement. Seul Pokémon n'en profite pas, faute de liste "Populaire" à subdiviser.
- Nouveau paramètre `maxSummaryLen` sur `POST /api/quiz-batch` et `POST /api/quiz-daily` : longueur de troncature des résumés désormais réglable côté client (100 à 1000 caractères, 1000 par défaut), à la place de l'ancienne constante fixe non paramétrable (800).
- Les articles Wikipédia de catégorie "histoire" (bataille, guerre) avec une date d'événement connue à la précision du jour près (Wikidata) peuvent maintenant apparaître dans le "quiz du jour" comme anniversaire ("Survenu il y a N ans"), au même titre qu'une sortie de film ou une naissance.
- Nouveau filtre "genre" (Homme/Femme/Non-binaire) partagé par personne/acteur/réalisateur/peintre/chef d'État, sourcé depuis TMDb ou Wikidata selon la personne.
- Fix : le libellé de rôle affiché à l'écran réponse d'une personne (ex. "Acteur") restait toujours à la forme masculine même pour une femme — accordé désormais au féminin quand une forme existe (ex. "Actrice", "Réalisatrice", "Politicienne").
- 4 nouveaux rôles "personne" pilotés par config : écrivain, scientifique, chanteur, astronaute (rejoignent politicien/sportif/acteur/réalisateur/peintre).
- Le type "article Wikipédia" gagne 4 nouveaux groupes de catégories (Architecture : château/pont/gratte-ciel/phare ; Musique : instrument ; Gastronomie : fromage/fruit/plat national/cocktail ; Jeux : jeu de société) ainsi que minéral et vaisseau spatial en Sciences. Son filtre géographique se regroupe désormais par sous-région (comme "pays") plutôt que par pays individuel (~190 cases à cocher auparavant).
- Musique : la notoriété (palier Obscur/Niche/Star) se calcule maintenant à partir du meilleur rang de classement obtenu (chart pays et/ou genre confondus) — Apple ne fournissant aucun score de popularité direct contrairement à TMDb/IGDB. Un throttle global corrige des `403` en rafale sur l'API iTunes, et la résolution des morceaux "Classiques" (artiste+titre → id) est désormais mise en cache en base pour éviter de re-rechercher chaque morceau à chaque refresh.
- Fix : le filtre "population" des pays ne renvoyait plus jamais aucun résultat depuis que mledoze/countries a retiré ce champ de son schéma — récupéré désormais via Wikidata (P1082).
- Fix : le libellé "Filmographie" manquait pour le type acteur en mode photo/résumé (présent seulement pour "réalisateur").
