...
- Actor "decade" categories (`person_decade_*`, by birth decade) work like the music decade categories: TMDb has no `/discover/person` filterable by birth date, so every actor already pulled into `person_popular`/`person_country_*` gets an extra `/person/{id}` call for their `birthday`, cached for ~30 days (a birth date never changes, so this is a one-time cost per actor rather than something repeated on every weekly refresh). An actor with no usable birth date is simply excluded from these categories.
