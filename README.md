...
- Persists everything — entities, images, filters, usage stats — in a single SQLite database (`cache/data.sqlite`)
...
- Actor "decade" filters (by birth decade): TMDb has no `/discover/person` filterable by birth date, so every actor needs an extra `/person/{id}` call for their `birthday`. That lookup runs in its own background warm loop (like the painter and country-photo ones), not inline in the main refresh pass — with thousands of actors this can take a while and would otherwise block the whole pool from becoming ready behind it. Birthdays are cached for ~30 days (they never change, so this is close to a one-time cost per actor). An actor with no usable birth date is simply excluded from these filters.
...
- With `imagesPerItem > 1`, each frame of a painter item shows a *different* painting by that same painter rather than repeating the same picture.
