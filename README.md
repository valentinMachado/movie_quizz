...
- Generates quiz batches from categories, decades, genres, and geography (continents, and — for movies, actors, music, and paintings — specific countries) across eight media types: movies, TV shows, actors, directors, video games, music, countries, and paintings
- Directors can be guessed from the posters of the movies they directed (`type: "director"`), separately from guessing an actor/director/painter's own photo (`type: "person"`, `role` filter)
- Movies and video games can also be guessed from their (redacted) synopsis, not just a poster (`questionType: "synopsis"`)
...
- With `imagesPerItem > 1`, each frame of a painter item shows a _different_ painting by that same painter rather than repeating the same picture.
...
- Director filters guess the **director**, from posters of the movies they directed (`type: "director"`) — distinct from guessing their own photo (`type: "person"`, `role: "director"`). A director's filmography is completed beyond the curated `movie` pool via their own TMDb credits, in a dedicated background warm loop, so guessing isn't limited to the handful of their films that happen to already be in the popularity/genre/decade/country pool.
